...
> **Which models work under ZDR?** In practice, **all** catalog models route through a ZDR-capable upstream when the header is set. A batch probe against the live Provider API (2026-08-17, all 55 catalog ids, `x-cmd-zdr: 1` on every request) returned **zero** `422 cmd_zdr_no_providers` — every model the account's plan could access streamed normally; the only non-200s were plan-level 403s (`MODEL_NOT_IN_PLAN`), which occur identically without the header. The `cmd_zdr_no_providers` mapping stays as a fail-closed safety net (some future catalog addition may lack a ZDR upstream), but it is not something you should hit in normal use. The plugin therefore does **not** annotate individual models with ZDR status in the picker — a static client-side snapshot cannot know the server's routing, and a wrong label is worse than no label.
